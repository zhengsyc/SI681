...
def main():
    ...
    for dir in dir_names:
        ...
        for img_name in img_list:
            count = count + 1
            img = cv2.imread(os.path.join(args.root_path, dir, img_name), cv2.IMREAD_GRAYSCALE)
            ...
            print("{}({}/{}). Time: {}".format(os.path.join(args.root_path, dir, img_name), count, len(img_list), end))
            save_feature(args.save_path, dir, img_name, features.data.cpu().numpy()[0])
...
def save_feature(save_path, dir, img_name, features):
    img_path = os.path.join(save_path, dir, img_name)
    img_dir  = os.path.join(save_path, dir)
